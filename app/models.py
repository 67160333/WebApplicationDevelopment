...
class ShopImage(Base):
    ...
    รูปมีสองชนิดและอยู่คนละที่
    ------------------------------------------------------------------
    1. **รูปตัวอย่างที่มากับโค้ด** — `filename` ขึ้นต้นด้วย `/`
       เช่น `/photos/spa-massage/01.webp` ไฟล์อยู่ใน `web/photos/` ซึ่ง commit
       เข้า repo จึงอยู่ถาวร คอลัมน์ `data` เป็น NULL

    2. **รูปที่เจ้าของร้านอัปโหลดเอง** — `filename` เป็นชื่อไฟล์ล้วน
       และ**ไบต์ของรูปอยู่ในคอลัมน์ `data`**

    ทำไมเก็บไบต์ลงฐานข้อมูล
    ------------------------------------------------------------------
    ตอนแรกเก็บเป็นไฟล์บนดิสก์ ซึ่งเป็นวิธีมาตรฐานและเร็วกว่า
    แต่โฮสต์ฟรีที่ใช้อยู่ไม่มีดิสก์ถาวร — ไฟล์ถูกล้างทุกครั้งที่เครื่องหลับแล้วตื่น
    ไม่ใช่แค่ตอน deploy เจ้าของร้านอัปรูปตอนเช้า ตกเย็นรูปหายหมด

    ฐานข้อมูลเป็นที่เดียวในระบบนี้ที่ข้อมูลอยู่ถาวรจริง จึงย้ายมาเก็บที่นี่
    แลกกับฐานข้อมูลที่โตเร็วขึ้น — ซึ่งรับได้เพราะรูปถูกย่อเหลือด้านยาวสุด
    1600 พิกเซลและแปลงเป็น WebP แล้ว เฉลี่ยใบละ 60–150 KB

    ถ้าวันหนึ่งย้ายไปโฮสต์ที่มีดิสก์ถาวรหรือใช้ S3/Cloudinary
    ให้เปลี่ยนที่ `app/storage.py` ที่เดียว ส่วนอื่นไม่ต้องแก้
    """
    ...
    # ไบต์ของรูป (WebP ที่ย่อแล้ว) — NULL สำหรับรูปตัวอย่างที่มากับโค้ด
    #
    # deferred=True บอก SQLAlchemy ว่า "อย่าดึงคอลัมน์นี้มาถ้าไม่ได้ขอ"
    # สำคัญมาก เพราะหน้าค้นหาอ่านรายการรูปของ 100 ร้าน ถ้าลากไบต์มาด้วยทุกครั้ง
    # จะดึงข้อมูลหลายสิบเมกะไบต์มาทิ้งเปล่า ๆ ทั้งที่ต้องการแค่ชื่อไฟล์
