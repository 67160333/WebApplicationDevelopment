...
# วันหยุดประจำสัปดาห์ของร้าน (0 = อาทิตย์ ... 6 = เสาร์ — ตรงกับ JS)
#
# เลือกใส่ให้เฉพาะบางหมวดที่ร้านจริงมักหยุด ไม่ใส่ให้ทุกร้าน
# เพราะถ้าร้านทุกร้านมีวันหยุด ตารางเวลาทำการจะดูเหมือนกันหมดจนไม่มีความหมาย
# และเว็บจะเหลือวันให้จองน้อยกว่าที่ควร
#
# ค่าเป็นรายหมวด ไม่ใช่รายชื่อร้าน — ชื่อร้านห้ามใช้เป็นกุญแจ
# เพราะระบบ seed ใช้ชื่อร้านตรวจซ้ำอยู่แล้ว (ดูกฎใน PROJECT_CONTEXT.md)
CLOSED_WEEKDAY_BY_CATEGORY: dict[str, str] = {
    "spa-massage": "1",        # หยุดจันทร์ — วันที่คนว่างน้อยที่สุดของร้านนวด
    "nail": "1",               # หยุดจันทร์
    "hair": "1",               # หยุดจันทร์ ตามธรรมเนียมร้านเสริมสวย
    "tattoo": "1,2",           # หยุดจันทร์–อังคาร งานสักรับคิวน้อยวันแต่ยาว
    "beauty-clinic": "0",      # หยุดอาทิตย์
    "mens-clinic": "0",        # หยุดอาทิตย์
    "car-care": "0",           # หยุดอาทิตย์ ช่างต้องได้พัก
}
...
def seed_closed_weekdays(db: Session) -> None:
    ...
    มีไว้เพื่อให้ฟีเจอร์ "เวลาทำการทั้งสัปดาห์" มีอะไรให้เห็นจริง
    ...
    **แตะเฉพาะร้านของบัญชีตัวอย่างเท่านั้น**
    ...
    for shop in rows:
        ...
        value = CLOSED_WEEKDAY_BY_CATEGORY.get(cat.slug if cat else "")
        if value:
            shop.closed_weekdays = value
            changed += 1
