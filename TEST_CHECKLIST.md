...
## 6. รอบเพดานการจอง + การชำระเงิน (21 ส.ค. 2026)

รอบนี้แก้ `app/routers/bookings.py` · `app/routers/payments.py` · `web/js/api.js` ·
`web/bookings.html` · `web/manage.html` · `web/shop.html`
→ **แตะโค้ดฝั่งเซิร์ฟเวอร์ ต้อง `docker compose up -d --build` ไม่ใช่แค่รีเฟรช**

### 6.1 รันชุดทดสอบอัตโนมัติ (เร็วที่สุด ใช้เวลาไม่ถึงนาที)
...
ต้องจบด้วย `สรุป: ผ่าน 22 · ไม่ผ่าน 0`

ชุดนี้ยิงคำขอเข้า API จริงบน SQLite ชั่วคราว **ไม่แตะฐานข้อมูลของเดโม**
ข้อมูลทดสอบถูกลบทิ้งเมื่อสคริปต์จบ

> **ทำไมต้อง `up -d api` ก่อน** — `Dockerfile` copy เข้า image แค่ `./app`
> โฟลเดอร์ `tests/` จึงไม่ได้อยู่ใน image แต่ถูก mount เข้าไปผ่าน `docker-compose.yml`
> container ที่สร้างไว้ก่อนหน้ายังไม่มี mount นี้ ต้องให้ compose สร้างใหม่หนึ่งครั้ง
> ถ้าข้ามขั้นนี้จะเจอ `can't open file '/app/tests/test_booking_payment.py'`
> ทำครั้งเดียวพอ ครั้งต่อ ๆ ไปสั่ง `exec` ได้เลย
>
> จงใจไม่ copy `tests/` ลง image เพราะ image ที่เอาไปรันจริง (รวมถึงบน Render)
> ไม่ควรมีไฟล์ทดสอบติดไปด้วย และการ mount ทำให้แก้ไฟล์ทดสอบแล้วรันใหม่ได้ทันที

### 6.2 กดจริงในเบราว์เซอร์

| # | ทำอะไร | ผลที่ต้องได้ |
|---|--------|-------------|
| 1 | หน้าจอง เลื่อนแถบวันที่ไปจนสุด | มีให้เลือก 14 วัน ไม่มีวันที่ไกลกว่านั้น |
| 2 | เปิด `/docs` ยิง `POST /api/bookings` ด้วยวันที่ปี 2081 | ได้ 422 พร้อมข้อความ "จองล่วงหน้าได้ไม่เกิน 90 วัน (ถึงวันที่ ...)" |
| 3 | หน้าการจองของฉัน กด **เลื่อนนัด** แล้วกดที่ช่องวันที่ | ปฏิทินเลือกได้แค่ถึง 90 วันข้างหน้า พิมพ์ปีเองก็ไม่ผ่าน |
| 4 | คิวในอนาคตที่ยังไม่จ่าย | ปุ่มเขียนว่า **"ชำระมัดจำ"** พร้อมยอดมัดจำ |
| 5 | คิวที่เลยเวลานัดไปแล้วและยังไม่จ่าย | ปุ่มเปลี่ยนเป็น **"ชำระค่าบริการ"** เก็บเต็มจำนวน ไม่ใช่มัดจำ |
| 6 | คิวที่จ่ายมัดจำแล้ว | ปุ่มเป็น **"ชำระยอดคงเหลือ"** ยอดตรงกับส่วนต่าง |
| 7 | ฝั่งร้าน กด **รับเงินสด** ให้คิวของเมื่อวาน | บันทึกได้ ออกใบเสร็จเต็มจำนวนในครั้งเดียว |
| 8 | ฝั่งร้าน ยิง `POST /api/payments/{id}/refund` ให้คิวที่ยัง **ยืนยันแล้ว** | ได้ 409 "ต้องยกเลิกคิวก่อนจึงจะคืนเงินได้" |
| 9 | ยกเลิกคิวนั้นก่อน แล้วคืนเงินอีกครั้ง | สำเร็จ · ลูกค้าได้แจ้งเตือน · ยอดที่จ่ายกลับเป็น 0 |
| 10 | กดคืนเงินรายการเดิมซ้ำ | ได้ 409 "รายการนี้คืนเงินไปแล้ว" |

### 6.3 ชุดถดถอยที่รันไปแล้ว

| ชุดตรวจ | ผล |
|---------|-----|
| การจอง + การชำระเงินผ่าน API จริง 22 ข้อ | ผ่าน 22 · ไม่ผ่าน 0 |
| การชนกันของคิวยาวหลายชั่วโมง 139 ข้อ | ผ่าน 139 · ไม่ผ่าน 0 |
| ตรรกะระบบจอง 13 ข้อ | ผ่าน 13 · ไม่ผ่าน 0 |
| ข้อความบนปุ่มชำระเงิน 3 สถานการณ์ + เพดานช่องวันที่ | ตรงกับกติกาเซิร์ฟเวอร์ทุกกรณี |
| ไวยากรณ์ JavaScript 12 ไฟล์ · คลาส CSS ที่ใช้แล้วไม่มีนิยาม | ผ่านครบ ไม่มีตัวไหนขาด |
| เปิดครบ 10 หน้า วัด CLS · เนื้อหาล้นจอ · error | สูงสุด 0.0313 — ไม่มี error |
| ผลเก่าทับผลใหม่ตอนกดตัวกรองรัว ๆ | ไม่เกิดขึ้นแล้วทั้งสองหน้า |

---

## 7. รอบกติกาการล็อกคิว + ค่าปรับยกเลิก (21 ส.ค. 2026)

รอบนี้ **เพิ่มคอลัมน์ในฐานข้อมูล** (`holds_slot` · `cancelled_by` · `cancellation_fee`)
ระบบเติมให้เองตอนสตาร์ต ไม่ต้องลบ volume และข้อมูลเดิมไม่หาย
...
ใน logs ต้องเห็นบรรทัดที่ขึ้นต้นว่า `ปรับโครงสร้างฐานข้อมูล` และชุดทดสอบต้องจบด้วย
`สรุป: ผ่าน 46 · ไม่ผ่าน 0`

### 7.1 กติกาข้อที่ 1 — ยังไม่จ่าย = ยังไม่ล็อกเวลา

ต้องเปิดสองหน้าต่าง ล็อกอินคนละบัญชี (`mind` กับ `nong` รหัส `Password123`)

| # | ทำอะไร | ผลที่ต้องได้ |
|---|--------|-------------|
| 1 | `mind` จองคิว เลือกช่างเจาะจง แล้วยังไม่จ่าย | การ์ดคิวขึ้นป้าย **"ยังไม่ล็อกเวลา"** สีเหลือง |
| 2 | `nong` เปิดหน้าร้านเดิม เลือกช่างคนเดิม วันเวลาเดิม | ช่องเวลานั้น**ยังกดได้** และจองสำเร็จ |
| 3 | `mind` กดชำระมัดจำ | สำเร็จ · ป้ายเปลี่ยนเป็น **"ล็อกเวลาให้แล้ว"** |
| 4 | รีเฟรชหน้าร้าน ดูช่องเวลาเดิม | ช่องนั้น**กดไม่ได้แล้ว** |
| 5 | `nong` กดชำระมัดจำคิวของตัวเอง | ถูกปฏิเสธ 409 · ข้อความบอกให้ไปกดเลื่อนนัด |
| 6 | อ่านกล่องยืนยันการจองในหน้าร้าน | เห็นกติกาสองข้อในกรอบเทา ก่อนกดปุ่มยืนยัน |

### 7.2 กติกาข้อที่ 2 — ยกเลิกเองเสียค่ามัดจำ

| # | ทำอะไร | ผลที่ต้องได้ |
|---|--------|-------------|
| 7 | `mind` กดยกเลิกคิวที่จ่ายมัดจำแล้ว | กล่องยืนยันขึ้นหัวข้อ **"ยกเลิกแล้วจะไม่ได้เงินมัดจำ ฿X คืน"** พร้อมตัวเลขจริง |
| 8 | ยืนยันยกเลิก | ข้อความตอบกลับบอกยอดที่ถูกหัก · การ์ดขึ้นบรรทัดสีเหลืองว่าถูกหักเท่าไหร่ |
| 9 | กลับไปดูหน้าร้าน | ช่องเวลานั้นกลับมาว่างให้คนอื่นจอง |
| 10 | ฝั่งร้านดูหน้าจัดการ | คิวนั้นขึ้น **"ได้ค่ามัดจำ ฿X"** สีเขียว · ไม่มีแจ้งเตือนค้างคืนเงิน |
| 11 | `mind` ยกเลิกคิวที่**ยังไม่จ่าย** | กล่องบอกว่า "ยกเลิกได้โดยไม่เสียค่าใช้จ่าย" · ไม่ถูกหักอะไร |
| 12 | ฝั่ง**ร้าน**กดยกเลิกคิวที่ลูกค้าจ่ายมัดจำแล้ว | กล่องเตือนว่าต้องคืนเงินเต็มจำนวน · ลูกค้าได้แจ้งเตือนว่าจะได้เงินคืนครบ |
| 13 | ฝั่งร้านกด "ยืนยันคิว" ให้คิวที่ยังไม่จ่าย | กล่องเตือนว่าการยืนยันจะกันเวลาให้ลูกค้ารายนี้ · กดแล้วป้ายเปลี่ยนเป็นล็อกแล้ว |

### 7.3 ผลตรวจอัตโนมัติรอบนี้

| ชุดตรวจ | ผล |
|---------|-----|
| การจอง + ชำระเงิน + ล็อกคิว + ค่าปรับ ผ่าน API จริง | ผ่าน 46 · ไม่ผ่าน 0 |
| การชนกันของคิวยาวหลายชั่วโมง | ผ่าน 139 · ไม่ผ่าน 0 |
| ตรรกะระบบจอง | ผ่าน 13 · ไม่ผ่าน 0 |
| ข้อความบนปุ่มชำระเงิน + เพดานช่องวันที่ | ตรงกับกติกาเซิร์ฟเวอร์ทุกกรณี |
| ไวยากรณ์ JavaScript 12 ไฟล์ · คลาส CSS ที่ใช้แล้วไม่มีนิยาม | ผ่านครบ |
| เปิดครบ 10 หน้า วัด CLS · เนื้อหาล้นจอ · error | สูงสุด 0.0424 — ไม่มี error |
| ผลเก่าทับผลใหม่ตอนกดตัวกรองรัว ๆ | ไม่เกิดขึ้น |
