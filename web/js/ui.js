...
const ICON = {
  ...
  utensils: '<path d="M6.5 3.5v6a2.5 2.5 0 0 0 5 0v-6"/><path d="M9 9.5v11"/><path d="M16.5 20.5v-7"/><path d="M16.5 13.5c2.2 0 3-1.6 3-4.5s-.8-5.5-3-5.5-3 2.6-3 5.5.8 4.5 3 4.5Z"/>',
  ...
  // สัญลักษณ์เพศชายที่มีกากบาทการแพทย์อยู่ข้างใน — คลินิกสุขภาพชาย
  mars: '<circle cx="9.8" cy="14.4" r="5.6"/><path d="M9.8 12v4.8M7.4 14.4h4.8"/><path d="m14.3 9.9 6.2-6.2"/><path d="M15.6 3.7h4.9v4.9"/>',
  // รถสกูตเตอร์ — บริการที่ช่างเดินทางไปหาลูกค้า
  scooter: '<circle cx="5.6" cy="17.2" r="3.2"/><circle cx="18.4" cy="17.2" r="3.2"/><path d="M8.8 17.2h6.4"/><path d="M15.2 17.2 12.4 8.4H9.9"/><path d="M12.4 8.4h4.3l1.7 8.8"/>',
  // รถยนต์มองจากด้านข้าง — ศูนย์ดูแลรถ
  car: '<path d="M2.8 16.2v-3.1l2.1-4.5a2 2 0 0 1 1.8-1.2h10.6a2 2 0 0 1 1.8 1.2l2.1 4.5v3.1"/><path d="M2.8 13.1h18.4"/><circle cx="7.1" cy="16.6" r="1.9"/><circle cx="16.9" cy="16.6" r="1.9"/>',
  // ไมโครโฟน — ห้องคาราโอเกะ
  mic: '<rect x="9.2" y="2.8" width="5.6" height="10.8" rx="2.8"/><path d="M5.6 11.6a6.4 6.4 0 0 0 12.8 0"/><path d="M12 18v3.2M9.2 21.2h5.6"/>',
};
...
const CATEGORY_ICON = {
  ...
  badminton: "badminton",
  delivery: "parcel",
  // หมวดที่เจาะกลุ่มผู้ชาย
  "mens-clinic": "mars",
  "mobile-barber": "scooter",
  "car-care": "car",
  karaoke: "mic",
};
...
// ============================================================
// กลุ่มใหญ่ 4 กลุ่ม — แยกทางเข้าไม่ให้ผู้ใช้สับสน
// ============================================================
// ปัญหาเดิม: หน้าแรกโยนหมวดทั้ง 12 มาเรียงกันเป็นแถวเดียว
// และหน้าค้นหาเอาสปากับสนามบอลมาปนกันในลิสต์เดียวโดยใช้ตัวกรองชุดเดียวกัน
// ผู้ใช้ที่มาหาสนามบอลต้องเลื่อนผ่านร้านทำเล็บ ซึ่งไม่มีเหตุผลอะไรเลย
//
// แบ่งตาม "รูปแบบการใช้บริการ" ไม่ใช่ตามอุตสาหกรรม เพราะผู้ใช้คิดแบบนั้น
// เขาไม่ได้ถามว่านี่คืออุตสาหกรรมความงามหรือกีฬา แต่ถามว่า
// "ต้องไปที่ร้าน หรือเขามาหาเรา" และ "ไปคนเดียวหรือไปเป็นกลุ่ม"
//
// ค่า key ต้องตรงกับ categories.group_key ในฐานข้อมูล (ดู seed_men.py)
const GROUPS = [
  {
    key: "care",
    name: "ดูแลตัวเอง",
      ...
      <div class="max-w-6xl mx-auto px-5 py-14">
        ...
        <div class="mt-12 pt-6" style="border-top:1px solid rgba(255,255,255,.1)">
          ...
          <div class="text-xs mt-3" style="color:rgba(255,255,255,.4);line-height:1.8">
            ร้านและรีวิวที่แสดงอยู่เป็นข้อมูลตัวอย่างสำหรับทดลองใช้ระบบ
            · ยังไม่เปิดรับชำระเงินออนไลน์ ยอดที่แสดงเป็นการบันทึกในระบบเท่านั้น
          </div>
        </div>
      </div>
    </footer>`;
...
/** วาดแถบลิงก์ไปกลุ่มอื่น ใต้หัวข้อของหน้ากลุ่ม
 *
 *  มีคำว่า "กลุ่มอื่น" นำหน้าเสมอ
 *  ------------------------------------------------------------------
 *  ของเดิมมีแต่ชิปสามอันลอยอยู่ใต้เส้นคั่น ไม่มีอะไรบอกว่ามันคืออะไร
 *  คนเปิดหน้า "ดูแลตัวเอง" แล้วเห็นคำว่า "กีฬา & สังสรรค์" อยู่ข้างล่าง
 *  จะอ่านว่าเป็นหมวดย่อยของหน้านี้ ซึ่งผิด — มันคือทางไปอีกหน้าหนึ่ง
 *  (มี aria-label อยู่แล้วแต่นั่นได้ยินเฉพาะโปรแกรมอ่านหน้าจอ คนที่มองเห็นไม่ได้อ่าน)
 *
 *  เดิมโค้ดชุดนี้ถูกคัดลอกไว้ในหน้ากลุ่มทั้งสี่หน้า แก้ทีต้องไล่แก้สี่ที่
 */
function renderGroupSwitch(currentKey) {
  const nav = document.querySelector(".grp-switch");
  if (!nav) return;
  nav.innerHTML =
    `<span class="grp-switch-label">กลุ่มอื่น</span>` +
    GROUPS.filter((g) => g.key !== currentKey)
      .map((g) => `<a href="${g.key}.html">${icon(g.icon, 15)} ${esc(g.name)}</a>`)
      .join("");
}
