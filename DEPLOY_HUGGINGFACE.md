...
> # ⚠️ ข้อมูลในไฟล์นี้ล้าสมัยแล้ว — อ่านตรงนี้ก่อน
>
> **Hugging Face เปลี่ยนนโยบายแล้ว — Docker Space ไม่ฟรีอีกต่อไป**
> เอกสารทางการระบุว่า Static Space เท่านั้นที่ฟรี ส่วน Gradio และ Docker Space
> ต้องสมัคร PRO (บัญชีส่วนตัว) หรือ Team/Enterprise (องค์กร) ก่อนถึงจะสร้างได้
> ตรวจสอบเมื่อ 20 ส.ค. 2026 ที่ https://huggingface.co/docs/hub/en/spaces-overview
>
> **โปรเจกต์นี้ต้องใช้ Docker Space** เพราะเป็น FastAPI + PostgreSQL ไม่ใช่หน้าเว็บนิ่ง ๆ
> จึงใช้วิธีในไฟล์นี้แบบฟรีไม่ได้แล้ว
>
> ### แล้วต้องทำยังไง
>
> **ข้อ 2 ของใบงานไม่ได้บังคับให้เอาเว็บขึ้นอินเทอร์เน็ต** — "base on GitHub" หมายถึง
> เดโมโค้ดชุดเดียวกับที่อยู่บน GitHub ซึ่งรันในเครื่องด้วย `docker compose up` ก็ครบแล้ว
> **อ่าน `DEMO_GUIDE.md` แทน** นั่นคือแผนหลักที่ควรใช้
>
> ถ้ายังอยากได้ลิงก์สาธารณะจริง ๆ ทางเลือกที่ยังใช้ได้ในปี 2026 คือ **Render.com**
> รองรับ Docker บนแพ็กเกจฟรี (512 MB RAM · หลับเมื่อไม่มีคนใช้ · ต้องผูกบัตรตอนสมัคร)
> ใช้คู่กับฐานข้อมูลจาก Neon ได้เหมือนเดิม ส่วน `Dockerfile.hf` กับโฟลเดอร์ `hf-upload`
> ยังใช้ต่อได้ทั้งชุด เพราะเป็น container เดียวที่เสิร์ฟทั้ง API และหน้าเว็บอยู่แล้ว
>
> **เนื้อหาด้านล่างเก็บไว้เป็นบันทึก** ขั้นตอน Neon (ขั้นที่ 1) ยังใช้ได้จริงทุกบรรทัด
> ส่วนขั้นที่ 2-5 ที่เกี่ยวกับ Hugging Face ใช้ไม่ได้แล้วถ้าไม่ได้สมัคร PRO
